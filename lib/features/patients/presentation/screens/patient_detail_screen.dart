  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final patientAsync = ref.watch(
      StreamProvider<Patient>(
        (ref) =>
            ref.watch(patientRepositoryProvider).watchPatientById(patientId),
      ),
    );
...
class _PatientDetailBodyState extends ConsumerState<_PatientDetailBody>
  ...
  void _showEditDialog(BuildContext context, Patient patient) {
    ...
    var selectedGender = patient.gender;
        ...
        GlassCard(
          child: Column(
            ...
            children: [
              ...
              _infoRow(context, LucideIcons.phone, 'Phone',
                  patient.phone ?? 'Not provided'),
              _infoRow(context, LucideIcons.user, 'Gender',
              ...
              _infoRow(
                  ...
                  patient.dateOfBirth != null
                      ...
                      : 'Not provided'),
              _infoRow(context, LucideIcons.mapPin, 'Address',
                  patient.address ?? 'Not provided'),
              const Divider(height: AppTheme.space24),
              _infoRow(context, LucideIcons.calendarDays, 'Registered',
                  dateFormat.format(patient.createdAt)),
              _infoRow(context, LucideIcons.refreshCw, 'Last Updated',
...
class _MedicalRecordsTab extends ConsumerWidget {
    ...
    final recordsAsync = ref.watch(
      StreamProvider<List<MedicalRecord>>(
        (ref) => ref
            ...
            .watchMedicalRecordsForPatient(patientId),
      ),
    );
        ...
        data: (records) => records.isEmpty
            ...
            : ListView.builder(
                ...
                itemBuilder: (context, index) =>
                    _buildRecordCard(context, ref, records[index]),
              ),
  ...
  Widget _buildRecordCard(
      BuildContext context, WidgetRef ref, MedicalRecord record) {
          ...
          children: [
            Row(
              children: [
                ...
                Text(dateFormat.format(record.visitDate),
                    style: TextStyle(
            ...
            if (record.prescriptionText != null &&
                record.prescriptionText!.isNotEmpty) ...[
              const SizedBox(height: AppTheme.space8),
              Text('Prescription',
                  style: TextStyle(
                      fontSize: 11,
              ...
              Container(
                ...
                child: Text(record.prescriptionText!,
          ...
          ElevatedButton(
            onPressed: () async {
              await ref.read(patientRepositoryProvider).addMedicalRecord(
                    patientId: patientId,
                    diagnosis:
                        diagnosisCtrl.text.isEmpty ? null : diagnosisCtrl.text,
                    notes: notesCtrl.text.isEmpty ? null : notesCtrl.text,
                    prescriptionText: prescriptionCtrl.text.isEmpty
                        ? null
                        : prescriptionCtrl.text,
                  );
              if (ctx.mounted) Navigator.pop(ctx);
            },
            child: const Text('Save'),
    ...
    return apptAsync.when(
      data: (appointments) => appointments.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  _buildApptCard(context, appointments[index]),
            ),
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (err, _) => Center(child: Text('Error: $err')),
    );
    ...
    return Padding(
      ...
      child: GlassCard(
        ...
        child: ListTile(
          ...
          title: Text(dateFormat.format(appt.datetime),
              style:
                  const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          subtitle: Text(appt.notes ?? 'No notes',
